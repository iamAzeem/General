...
enum TestDefaults
{
    GLOBAL_HASHMAP_SIZE     = 10,
    MAP_ENTRIES_AT_STARTUP  = 100,
...
bool setupTestEnvironment( void )
{
    ...
    for ( size_t i = 0; i < MAP_ENTRIES_AT_STARTUP; ++i )
    {
        const TestType key = i + 1;
        const TestType val = rand() % MAP_ENTRIES_AT_STARTUP;
...
void writerCallback( void )
{
    ...
    for ( size_t i = 0; i < NUM_OF_WRITER_INTERVALS; ++i )
    {
        ...
        const TestType key = rand() % MAP_ENTRIES_AT_STARTUP;
        const TestType val = rand() % MAP_ENTRIES_AT_STARTUP;
...
void readerCallback( void )
{
    ...
    for ( size_t i = 0; i < NUM_OF_READER_INTERVALS; ++i )
    {
        ...
        TestType key = rand() % MAP_ENTRIES_AT_STARTUP;
        TestType val = 0;
...
//    /* Resize test */
//    for ( HashType k = 0, v = 100; k < 10; ++k, ++v )
//    {
//        globalHashMap.add( k, v );
//    }
//
//    cout << "Size: " << globalHashMap.size() << ", Length: " << globalHashMap.length() << endl;
//    globalHashMap.print();
//
//    cout << "Resizing..." << endl;
//    globalHashMap.resize( 20 );
//
//    /* After resizing */
//    cout << "After resize:" << endl;
//    for ( HashType k = 20, v = 100; k < 20; ++k, ++v )
//    {
//        globalHashMap.add( k, v );
//    }
//
